...
export const POST: RequestHandler = async ({ request, locals, cookies }) => {
  try {
    ...
    try {
      ...
      console.error("POST /api/reviews/publish - Failed to parse request body:", parseError)
      return json({
    ...
    console.log("POST /api/reviews/publish - Creating GoogleMyBusinessWrapperV3")
    ...
    try {
      ...
      console.error("POST /api/reviews/publish - Wrapper creation failed:", wrapperError)
      return json({
        error: "Failed to initialize Google service wrapper",
        details: wrapperError instanceof Error ? wrapperError.message : String(wrapperError)
      }, { status: 500 })
    }

    console.log("POST /api/reviews/publish - Attempting to create Google service")
    try {
      googleService = await googleWrapper.createService(tenant.id)
      console.log("POST /api/reviews/publish - Service creation completed, result:", !!googleService)
    } catch (serviceError) {
      console.error("POST /api/reviews/publish - Service creation threw error:", serviceError)
      return json({
    ...
    console.log("POST /api/reviews/publish - Google service created successfully")
    ...
    try {
      ...
      console.log("POST /api/reviews/publish - ResponsePublisherService created successfully")
    } catch (publisherError) {
      console.error("POST /api/reviews/publish - ResponsePublisherService creation failed:", publisherError)
      return json({
        error: "Failed to initialize publisher service",
        details: publisherError instanceof Error ? publisherError.message : String(publisherError)
      }, { status: 500 })
    }
    ...
    try {
      ...
      switch (action) {
        case "publish-single": {
          console.log("POST /api/reviews/publish - Publishing single response:", responseId)
          // Publish a single response immediately
          const result = await publisherService.publishSingleResponse(responseId, user?.id)
          console.log("POST /api/reviews/publish - Publish result:", result)

          if (!result.success) {
            console.error("POST /api/reviews/publish - Publish failed:", result.error)
            return json(
      ...
      case "queue-single": {
        console.log("POST /api/reviews/publish - Queueing single response:", responseId)
        ...
        console.log("POST /api/reviews/publish - Fetching AI response:", responseId)
        ...
        console.log("POST /api/reviews/publish - AI response fetch result:", { aiResponse, fetchError })

        if (fetchError || !aiResponse) {
          console.error("POST /api/reviews/publish - AI response not found:", fetchError)
        ...
        if (!locationId) {
          console.error("POST /api/reviews/publish - Missing location_id:", aiResponse)
          return json({ error: "Location ID not found for this response" }, { status: 400 })
        }

        // Add to queue with required fields based on actual schema
        console.log("POST /api/reviews/publish - Inserting into queue with data:", {
          tenant_id: tenant.id,
        ...
        if (queueError) {
          console.error("POST /api/reviews/publish - Queue insertion failed:", queueError)
          return json({ error: queueError.message }, { status: 500 })
        }

        console.log("POST /api/reviews/publish - Successfully queued response")
      ...
      console.error("POST /api/reviews/publish - Error in switch statement:", switchError)
      return json({ 
        error: "Failed to process publish request", 
        details: switchError instanceof Error ? switchError.message : String(switchError)
      }, { status: 500 })
    }
  } catch (err) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "process-queue": {
        ...
        const result = await publisherService.processBatch(tenant.id, 5, user?.id)
        return json(result)
      }
