<script lang="ts">
  ...
  interface Props {
    data: { user: User; profile: Profile; isInvitedUser?: boolean }
    form: FormAccountUpdateResult
  }

  let { data, form }: Props = $props()

  let { user, profile, isInvitedUser } = data
...
<svelte:head>
  <title>{isInvitedUser ? "Complete Profile" : "Create Profile"}</title>
</svelte:head>
  ...
  <div class="flex flex-col w-64 lg:w-80">
    <div>
      <h1 class="text-2xl font-bold mb-6">
        {isInvitedUser ? "Complete Your Profile" : "Create Profile"}
      </h1>
      {#if isInvitedUser}
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <p class="text-blue-800 text-sm">
            Welcome! You've successfully joined the team. Just complete your
