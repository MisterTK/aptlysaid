...
export async function decryptToken(encryptedTokenData) {
  ...
  const keysToTry = configuredKey && configuredKey !== DEFAULT_KEY ? [
    DEFAULT_KEY,
    configuredKey
  ] : [
    DEFAULT_KEY
  ...
  if (typeof encryptedTokenData === "string") {
    ...
    throw new Error(`Unsupported encrypted token format: ${typeof encryptedTokenData}`);
  }
  if (encryptedData.length < 32) {
    throw new Error(`Invalid encrypted token length: ${encryptedData.length} bytes`);
  }
  ...
  for (const encryptionKey of keysToTry){
    try {
      ...
      if (paddingLength > 0 && paddingLength <= 16) {
        ...
        for(let i = decryptedArray.length - paddingLength; i < decryptedArray.length; i++){
  ...
  throw new Error(`Token decryption failed: ${lastError?.message || 'Unknown error'}`);
}
export async function encryptToken(plainTextToken) {
  ...
  const paddedKey = encryptionKey.padEnd(32, "0").slice(0, 32);
  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(paddedKey), {
    name: "AES-CBC"
  }, false, [
    "encrypt"
  ]);
  const encrypted = await crypto.subtle.encrypt({
    name: "AES-CBC",
    iv
  }, key, new TextEncoder().encode(plainTextToken));
