...
export async function syncReviewToDatabase(review, locationId, tenantId, supabase) {
  ...
  const { error } = await supabase.from("reviews").upsert({
    ...
    metadata: {
      ...
      normalized_from: review.name !== normalizedReviewId ? review.name : null,
...
export async function getSyncState(supabase, tenantId, locationId) {
  const { data: location } = await supabase.from("locations").select("metadata").eq("id", locationId).single();
