...
export class GMBClient {
  ...
  async getReviews(locationName, daysBack = 30, pageToken, lastReviewTime) {
    ...
    if (!response.ok) {
      ...
      if (this.accountId && this.accountId !== "-" && !locationName.startsWith("accounts/")) {
        const wildcardPath = `accounts/-/locations/${locationName}/reviews`;
        const wildcardUrl = new URL(`https://mybusiness.googleapis.com/v4/${wildcardPath}`);
  ...
  filterReviewsByDate(data, daysBack, lastReviewTime) {
    ...
    const effectiveCutoff = lastReviewTime ? new Date(Math.max(new Date(lastReviewTime).getTime(), cutoffDate.getTime())) : cutoffDate;
  ...
  async listLocations() {
    ...
    if (!wildcardResponse.ok) {
      ...
      throw new Error(`Business Profile API error: ${wildcardResponse.statusText}`);
    }
  ...
  async replyToReview(reviewName, comment) {
    ...
    const response = await fetch(`https://mybusiness.googleapis.com/v4/${encodedReviewName}/reply`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${this.accessToken}`,
        "Content-Type": "application/json"
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to publish reply: ${response.statusText}. Google's response: ${errorBody}`);
    }
  }
