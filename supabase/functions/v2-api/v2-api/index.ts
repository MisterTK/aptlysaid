...
async function getUserTenant(userId, supabase) {
  const { data, error } = await supabase.from("tenant_users").select("tenant_id, role").eq("user_id", userId).eq("status", "active").single();
  if (error || !data) {
...
async function canManageResponses(userId, tenantId, supabase) {
  const { data } = await supabase.from("tenant_users").select("role").eq("user_id", userId).eq("tenant_id", tenantId).eq("status", "active").single();
  return [
    "owner",
    "admin",
    "manager"
...
async function getOAuthToken(tenantId, supabase) {
  const { data, error } = await supabase.from("oauth_tokens").select("*").eq("tenant_id", tenantId).eq("provider", "google").eq("provider_scope", "https://www.googleapis.com/auth/business.manage").eq("status", "active").single();
  if (error || !data) {
...
async function triggerTokenRefresh(tenantId, supabase) {
  ...
  await createWorkflow(WORKFLOW_TYPES.TOKEN_REFRESH, {
    tenantId,
    priority: 100
  }, tenantId, "system", supabase);
}
// ====================================================================
// WORKFLOW MANAGEMENT
// ====================================================================
...
async function createWorkflow(workflowType, context, tenantId, userId, supabase) {
...
async function handleSyncReviews(body, tenantId, userId, supabase) {
  ...
  const { data: location } = await supabase.from("locations").select("id, name, oauth_token_id").eq("id", locationId).eq("tenant_id", tenantId).single();
  if (!location) {
  ...
  const workflowId = await createWorkflow(WORKFLOW_TYPES.REVIEW_SYNC, {
    locationId,
    tenantId,
    userId,
    syncOptions: options,
...
async function handleGenerateResponse(body, tenantId, userId, supabase) {
  ...
  const { data: review } = await supabase.from("reviews").select("id, customer_name, rating, review_text").eq("id", actualReviewId).eq("tenant_id", tenantId).single();
  if (!review) {
    throw new Error("Review not found or access denied");
  }
  // Use review_processing workflow type for compatibility
  const workflowId = await createWorkflow("review_processing", {
...
async function handlePublishResponse(body, tenantId, userId, supabase) {
  ...
  const { data: response } = await supabase.from("ai_responses").select("id, status, review_id, response_text").eq("id", responseId).eq("tenant_id", tenantId).single();
  if (!response) {
  ...
  const workflowId = await createWorkflow(WORKFLOW_TYPES.RESPONSE_PUBLISHING, {
    responseId,
...
async function handleBatchGenerate(body, tenantId, userId, supabase) {
  ...
  const { data: reviews, error } = await supabase.from("reviews").select("id").in("id", reviewIds).eq("tenant_id", tenantId);
  if (error || reviews.length !== reviewIds.length) {
    throw new Error("Some reviews not found or access denied");
  }
  const workflowId = await createWorkflow(WORKFLOW_TYPES.BATCH_PROCESSING, {
    reviewIds,
    tenantId,
    userId,
...
async function handleWorkflowStatus(body, tenantId, supabase) {
  ...
  const { data: workflow, error } = await supabase.from("workflows").select(`
      ...
      completed_at,
      retry_count
    `).eq("id", workflowId).eq("tenant_id", tenantId).single();
  if (error || !workflow) {
    throw new Error("Workflow not found or access denied");
  }
  // Get recent steps
  const { data: steps } = await supabase.from("workflow_steps").select("step_name, status, started_at, completed_at, error_message").eq("workflow_id", workflowId).order("started_at", {
    ascending: false
  }).limit(10);
  return {
    success: true,
...
async function handleOAuthStatus(tenantId, supabase) {
  ...
  return {
    ...
    message: needsReauth ? "Token requires re-authentication" : expiring ? "Token expiring soon, refresh scheduled" : "Connection active"
  };
}
// ====================================================================
...
async function handleGetBusinessGuidance(tenantId, supabase) {
  const { data: guidance } = await supabase.from("business_guidance").select("*").eq("tenant_id", tenantId).single();
  return {
    success: true,
    data: {
      guidance
...
async function handleGetUpsellItems(tenantId, supabase) {
  const { data: items } = await supabase.from("upsell_items").select("*").eq("tenant_id", tenantId).eq("status", "active").order("priority", {
    ascending: false
  });
  return {
    success: true,
...
async function handleGetResponseSettings(tenantId, supabase) {
  const { data: settings } = await supabase.from("response_settings").select("*").eq("tenant_id", tenantId).single();
  return {
    success: true,
    data: {
      settings
...
async function handleGetResponseQueue(tenantId, supabase) {
  const { data: queue } = await supabase.from("response_queue").select(`
      *,
...
async function handleGetTeamMembers(tenantId, supabase) {
  const { data: members } = await supabase.from("tenant_users").select(`
      *,
...
async function handleGetInvitations(tenantId, supabase) {
  const { data: invitations } = await supabase.from("tenant_invitations").select("*").eq("tenant_id", tenantId).eq("status", "pending").gt("expires_at", new Date().toISOString()).order("created_at", {
    ascending: false
  });
  return {
    success: true,
...
async function handleGetOnboardingProgress(tenantId, supabase) {
  const { data: tenant } = await supabase.from("tenants").select("onboarding_completed, onboarding_step, onboarding_data").eq("id", tenantId).single();
  return {
    success: true,
    data: {
      progress: tenant
...
async function handleGetResponseMetrics(tenantId, supabase) {
  const { data: metrics } = await supabase.from("response_analytics").select("*").eq("tenant_id", tenantId).order("period_start", {
    ascending: false
  }).limit(30);
  return {
    success: true,
...
async function handleGetReviews(tenantId, supabase) {
  const { data: reviews } = await supabase.from("reviews").select(`
    ...
    `).eq("tenant_id", tenantId).order("review_date", {
    ascending: false
  }).limit(50);
  return {
    success: true,
...
async function handleGetLocations(tenantId, supabase) {
  const { data: locations } = await supabase.from("locations").select("*").eq("tenant_id", tenantId).order("name");
  return {
    success: true,
    data: {
      locations
...
async function handleGetWorkflows(tenantId, supabase) {
  const { data: workflows } = await supabase.from("workflows").select("*").eq("tenant_id", tenantId).order("created_at", {
    ascending: false
  }).limit(20);
  return {
    success: true,
...
async function handleGetPendingResponses(tenantId, supabase) {
  const { data: responses } = await supabase.from("ai_responses").select(`
      *,
      review:review_id(
        review_text,
        rating,
    ...
    `).eq("tenant_id", tenantId).eq("status", "draft").order("created_at", {
    ascending: false
  });
  return {
    success: true,
...
async function handleGetAIResponse(responseId, tenantId, supabase) {
  const { data: aiResponse, error } = await supabase.from("ai_responses").select("*").eq("id", responseId).eq("tenant_id", tenantId).single();
  if (error || !aiResponse) {
...
async function handleApproveResponse(body, tenantId, userId, supabase) {
  ...
  const { data: aiResponse } = await supabase.from("ai_responses").select("*, review:review_id(id)").eq("id", actualResponseId).eq("tenant_id", tenantId).single();
  if (!aiResponse) {
    throw new Error("Response not found");
  }
  if (approved !== false) {
    // Approve the response
    const { error: updateError } = await supabase.from("ai_responses").update({
      status: "approved",
    ...
    const { data: workflow } = await supabase.from("workflows").select("id").eq("tenant_id", tenantId).eq("workflow_type", "response_generation").eq("current_step", "wait_for_approval").eq("context_data->reviewId", aiResponse.review.id).single();
    if (workflow) {
...
async function handleUpdateBusinessGuidance(body, tenantId, supabase) {
  const { data, error } = await supabase.from("business_guidance").upsert({
    tenant_id: tenantId,
    ...body,
...
async function handleCreateUpsellItem(body, tenantId, supabase) {
  const { data, error } = await supabase.from("upsell_items").insert({
    tenant_id: tenantId,
    ...body
  }).select().single();
  if (error) {
...
async function handleUpdateUpsellItem(itemId, body, tenantId, supabase) {
  ...
  const { data: existing } = await supabase.from("upsell_items").select("id").eq("id", itemId).eq("tenant_id", tenantId).single();
  if (!existing) {
    throw new Error("Item not found");
  }
  const { data, error } = await supabase.from("upsell_items").update(body).eq("id", itemId).select().single();
  if (error) {
    throw new Error(error.message);
  }
  return {
...
async function handleDeleteUpsellItem(itemId, tenantId, supabase) {
  ...
  const { error } = await supabase.from("upsell_items").delete().eq("id", itemId);
  if (error) {
    throw new Error(error.message);
  }
  return {
...
async function handleUpdateResponseSettings(body, tenantId, supabase) {
  const { data, error } = await supabase.from("response_settings").upsert({
    tenant_id: tenantId,
    ...body
  }).select().single();
  if (error) {
...
async function handleAdvanceOnboarding(body, tenantId, supabase) {
  ...
  const { data: tenant } = await supabase.from("tenants").select("onboarding_data, onboarding_step").eq("id", tenantId).single();
  const updatedData = {
...
async function handleQueueApprovedResponses(body, tenantId, supabase) {
  ...
  let query = supabase.from("ai_responses").select("id, review_id, reviews!inner(location_id)").eq("tenant_id", tenantId).eq("status", "approved").is("published_at", null);
...
async function handleSyncLocation(body, tenantId, userId, supabase) {
  ...
  const { data: location } = await supabase.from("locations").select("id, name").eq("id", locationId).eq("tenant_id", tenantId).single();
  if (!location) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!tenantId) {
      const { data: existingGuidance } = await supabase.from("business_guidance").select("tenant_id").limit(1).single();
    ...
    if (restrictedEndpoints.includes(path)) {
      const hasPermission = await canManageResponses(user.id, tenantId, supabase);
      if (!hasPermission) {
    ...
    switch(true){
      ...
      case path === "/sync-reviews" && req.method === "POST":
        result = await handleSyncReviews(await req.json(), tenantId, user.id, supabase);
        break;
      case path === "/generate-response" && req.method === "POST":
        result = await handleGenerateResponse(await req.json(), tenantId, user.id, supabase);
        break;
      case path === "/publish-response" && req.method === "POST":
        result = await handlePublishResponse(await req.json(), tenantId, user.id, supabase);
        break;
      case path === "/batch-generate" && req.method === "POST":
        result = await handleBatchGenerate(await req.json(), tenantId, user.id, supabase);
        break;
      case path === "/workflow-status" && req.method === "POST":
        result = await handleWorkflowStatus(await req.json(), tenantId, supabase);
        break;
      case path === "/approve-response" && req.method === "POST":
        {
          ...
          result = await handleApproveResponse(await req.json(), tenantId, user.id, supabase);
          break;
        }
      case path === "/business-guidance" && req.method === "POST":
        result = await handleUpdateBusinessGuidance(await req.json(), tenantId, supabase);
        break;
      case path === "/upsell-items" && req.method === "POST":
        result = await handleCreateUpsellItem(await req.json(), tenantId, supabase);
        break;
      case path === "/response-settings" && req.method === "POST":
        result = await handleUpdateResponseSettings(await req.json(), tenantId, supabase);
        break;
      case path === "/invite-member" && req.method === "POST":
        {
          ...
          result = await handleInviteMember(await req.json(), tenantId, user.id, supabase);
          break;
        }
      case path === "/advance-onboarding" && req.method === "POST":
        result = await handleAdvanceOnboarding(await req.json(), tenantId, supabase);
        break;
      case path === "/queue-approved-responses" && req.method === "POST":
        result = await handleQueueApprovedResponses(await req.json(), tenantId, supabase);
        break;
      case path === "/sync-location" && req.method === "POST":
        result = await handleSyncLocation(await req.json(), tenantId, user.id, supabase);
        break;
      ...
      case path === "/reviews/publish" && req.method === "PUT":
        {
          ...
          if (putBody.action === "queue-all-approved") {
            ...
            const { error } = await supabase.from("response_queue").delete().eq("tenant_id", tenantId).eq("status", "failed");
            if (error) {
              throw new Error(error.message);
            }
            result = {
      ...
      case path.startsWith("/upsell-items/") && req.method === "PUT":
        {
          ...
          result = await handleUpdateUpsellItem(itemId, await req.json(), tenantId, supabase);
          break;
        }
    ...
    const statusCode = error.message.includes("not found") ? 404 : error.message.includes("denied") || error.message.includes("permission") ? 403 : error.message.includes("Authentication") ? 401 : 400;
    return new Response(JSON.stringify({
      success: false,
