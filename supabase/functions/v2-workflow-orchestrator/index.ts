...
class ExternalIntegratorClient {
  ...
  constructor(baseUrl, authToken){
    ...
    this.generateResponse = (reviewContent, tenantSettings)=>this.request("generate-ai-response", {
        reviewContent,
        tenantSettings
      });
    this.publishToGMB = (gmbReviewId, responseContent, tenantId)=>this.request("publish-to-gmb", {
        gmbReviewId,
        responseContent,
        tenantId
      });
    this.syncGMBReviews = (locationId, tenantId)=>this.request("sync-gmb-reviews", {
        locationId,
        tenantId
      });
    this.syncLocations = (tenantId, tokenId)=>this.request("sync-locations", {
        tenantId,
        tokenId
      });
    this.refreshOAuthToken = (tenantId)=>this.request("refresh-oauth-token", {
        tenantId
      });
  }
  async request(action, data) {
    return retryWithBackoff(async ()=>{
      ...
      if (!response.ok) {
        const error = new Error(`External integrator error: ${response.statusText}`);
...
const workflowSteps = {
  async generateAIResponse (ctx, supabase) {
    ...
    if (!baseUrl || !serviceKey) {
      throw new Error("Missing environment configuration for external integrator");
    }
    ...
    const { data: newResponse } = await supabase.from("ai_responses").insert({
      ...
      tone: review.rating >= 4 ? "grateful" : review.rating <= 2 ? "empathetic" : "professional",
      status: "draft",
      ai_model: aiResponse.model || "unknown",
      generation_tokens: aiResponse.metadata?.generationTokens || aiResponse.metadata?.totalTokens,
  ...
  async waitForApproval (ctx, supabase) {
    ...
    if (!shouldAutoPublish) {
      if (response.status === "draft") {
        throw new Error(`Waiting for manual approval - rating ${rating} not configured for auto-publish`);
      }
      return {
        approved: false,
    ...
    if (confidenceScore < (settings.min_confidence_score || 0.8)) {
      if (settings.require_human_review_below_threshold) {
        if (response.status === "draft") {
          throw new Error(`Waiting for manual approval - confidence score ${confidenceScore} below threshold ${settings.min_confidence_score}`);
        }
        return {
          approved: false,
    ...
    if (qualityScore < (settings.min_quality_score || 0.7)) {
      if (settings.require_human_review_below_threshold) {
        if (response.status === "draft") {
          throw new Error(`Waiting for manual approval - quality score ${qualityScore} below threshold ${settings.min_quality_score}`);
        }
        return {
          approved: false,
    ...
    if (settings.max_response_length && response.response_text.length > settings.max_response_length) {
      if (response.status === "draft") {
        throw new Error(`Waiting for manual approval - response length ${response.response_text.length} exceeds limit ${settings.max_response_length}`);
      }
      return {
        approved: false,
    ...
    if (settings.excluded_keywords?.length > 0) {
      ...
      const hasExcludedKeyword = settings.excluded_keywords.some((keyword)=>responseText.includes(keyword.toLowerCase()));
      if (hasExcludedKeyword) {
        if (response.status === "draft") {
          throw new Error("Waiting for manual approval - response contains excluded keywords");
        }
        return {
          approved: false,
    ...
    if (settings.working_hours_only && settings.working_hours) {
      ...
      if (currentHour < todayHours.start || currentHour >= todayHours.end) {
        if (response.status === "draft") {
          throw new Error(`Waiting for manual approval - outside working hours (${todayHours.start}-${todayHours.end})`);
        }
        return {
          approved: false,
          reason: "outside_working_hours"
    ...
    if (!isManuallyApproved) {
      ...
      if (settings) {
        ...
        if (!publishingAllowed) {
          throw new Error(`Publishing not allowed - rating ${rating} not configured for auto-publish.`);
        }
        // Check rate limits
        if (settings.rate_limits) {
          ...
          if (rateLimits.daily_limit || rateLimits.hourly_limit) {
            ...
            if (rateLimits.daily_limit && todayCount >= rateLimits.daily_limit) {
              throw new Error(`Daily publishing limit reached: ${todayCount}/${rateLimits.daily_limit}`);
            }
            if (rateLimits.hourly_limit && hourCount >= rateLimits.hourly_limit) {
              throw new Error(`Hourly publishing limit reached: ${hourCount}/${rateLimits.hourly_limit}`);
            }
          }
        }
        ...
        if (settings.working_hours_only && settings.working_hours) {
          ...
          const workingHours = settings.working_hours;
          const todayHours = workingHours[currentDay] || workingHours['default'];
          if (!todayHours?.enabled || currentHour < todayHours.start || currentHour >= todayHours.end) {
            throw new Error(`Cannot publish outside working hours (${todayHours?.start || 9}-${todayHours?.end || 17})`);
          }
        }
      }
    }
  ...
  async refreshToken (ctx, supabase) {
    ...
    try {
      ...
      throw new Error(`Failed to decrypt refresh token: ${decryptError instanceof Error ? decryptError.message : 'Unknown error'}`);
    }
    // Refresh the token with Google
    const clientId = Deno.env.get("PUBLIC_GOOGLE_CLIENT_ID") || Deno.env.get("GOOGLE_CLIENT_ID") || "";
    const clientSecret = Deno.env.get("GOOGLE_CLIENT_SECRET") || "";
    if (!clientId || !clientSecret) {
      throw new Error(`Missing Google OAuth credentials - clientId: ${clientId ? 'present' : 'missing'}, clientSecret: ${clientSecret ? 'present' : 'missing'}`);
    }
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (action === "process_pending_workflows") {
      ...
      const { data: workflows } = await supabase.from("workflows").select("*").in("status", [
        "pending",
        "running"
      ...
      for (const workflow of workflows){
        ...
        try {
          ...
          if (workflow.workflow_type === 'response_publishing' && workflow.input_data?.queueItemId) {
            ...
            if (errorMessage.includes("NOT_FOUND")) {
              console.log(`Handling NOT_FOUND error for queue item ${queueItemId}`);
              ...
              const { data: queueItem } = await supabase.from("response_queue").select("attempt_count, max_attempts").eq("id", queueItemId).single();
              if (queueItem) {
                const newAttemptCount = (queueItem.attempt_count || 0) + 1;
                const maxAttempts = queueItem.max_attempts === null || queueItem.max_attempts === undefined ? 3 : queueItem.max_attempts;
                ...
                await supabase.from("response_queue").update({
                  ...
                  next_retry_at: canRetry ? new Date(Date.now() + Math.pow(2, newAttemptCount - 1) * 60000).toISOString() : null
                }).eq("id", queueItemId);
              }
            }
          }
